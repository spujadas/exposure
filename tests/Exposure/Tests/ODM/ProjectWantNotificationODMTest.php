...
class ProjectWantNotificationODMTest extends \PHPUnit_Framework_TestCase {
    protected static $dm ;
    ...
    public function setUp() {
        ...
        $this->notification->setWant($this->want);
        $this->notification->validate();
        
        self::$dm->persist($this->notification);
        self::$dm->persist($this->want);
        self::$dm->flush();
        
        self::$id = $this->notification->getId();
        self::$wantId = $this->want->getId();
    ...
    public function testFound() {
        $this->notification = 
                ObjectDocumentMapper::getById(self::$dm, 'Exposure\Model\ProjectWantNotification', self::$id);
        $this->assertNotNull($this->notification);
        $this->assertInstanceOf('Exposure\Model\ProjectWantNotification', $this->notification);
    }
    ...
    public function testIsEqual() {
        ...
        $this->assertEquals($this->want->getId(), $notification->getWant()->getId());
    ...
    public function testRemove() {
        ...
        $this->assertNull(ObjectDocumentMapper::getById(self::$dm, 'Exposure\Model\ProjectWantNotification', self::$id));
    }
    ...
    public static function cleanUp() {
        ...
        if (!is_null(self::$wantId)) {
            $want = ObjectDocumentMapper::getById(self::$dm, 'Exposure\Model\ProjectWant', self::$wantId);
            if(!is_null($want)) {
                self::$dm->remove($want);
            }
        }
